<template>
    <div v-if="dispList.filter(x => x.room == '').length > 0">
        <h2>No room</h2>
        <div v-if="dispList.filter(x => x.room == '' && x.type == 'sensor').length > 0">
            <p><b>Sensores</b></p>
        </div>
        ...
        <div v-if="dispList.filter(x => x.room == '' && x.type == 'ejecutor').length > 0">
            <p><b>Ejecutores</b></p>
        </div>
        <div class="dispositivo" v-for="(dispositivo, index) in dispList.filter(x => x.room == '' && x.type == 'ejecutor')"
            :key="index">
            <p>{{ dispositivo.name }}</p>
